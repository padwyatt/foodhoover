...
from scrape_bq import bq_get_places, bq_run_scrape, bq_insert_new_rx, bq_update_ue_sectors, bq_crawl_roo, bq_agg_results_district, bq_agg_results_sector, bq_update_geos, bq_agg_rx_results_fast, bq_step_logger, bq_agg_results_country, bq_export_rx_cx_results,bq_export_rx_ref,bq_export_agg_sector_run, bq_export_agg_district_run, bq_export_agg_country_run
...
def bq_post_process(steps, run_id):
    results = []
    if steps == ['ALL']:
        steps = ['INSERT','UPDATE-ROO','UPDATE-UE','UPDATE-GEOS','GET-PLACES','AGG-RESULTS-DISTRICT','AGG-RESULTS-SECTOR', 'RX-RESULTS-FAST','AGG-RESULTS-COUNTRY','EXPORT-RX-CX-RESULTS','EXPORT-RX-REF','EXPORT-AGG-SECTOR-RUN','EXPORT-AGG-DISTRICT-RUN','EXPORT-AGG-COUNTRY-RUN']
    for step in steps:
        if step == 'INSERT':
